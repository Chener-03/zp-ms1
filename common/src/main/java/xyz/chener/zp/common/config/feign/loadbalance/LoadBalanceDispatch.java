...
public class LoadBalanceDispatch implements ReactorServiceInstanceLoadBalancer {
    ...
    private Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances
        ...
        if (request.getContext() instanceof RequestDataContext rdc){
            ...
            if (!tags.isEmpty() && StringUtils.hasText(tags.getFirst())){
                tag = tags.getFirst();
                List<ServiceInstance> list = instances.stream().filter(e -> ObjectUtils.nullSafeEquals(e.getMetadata().get(HTTP_TAG_HEADER), tags.getFirst())).toList();

                if (list.isEmpty()){
                    instances = instances.stream().filter(ins-> !StringUtils.hasText(ins.getMetadata().get(HTTP_TAG_HEADER))).toList();
                }else {
                    instances = list;
                }
        ...
        if (instances.isEmpty() && nextInstance == null) {
            if (log.isWarnEnabled()) {
                if (tag != null){
                    log.warn("No servers available for tag service: " + tag + ":::" + serviceId);
                }else {
                    log.warn("No servers available for no tag service: " + serviceId);
                }
            }
